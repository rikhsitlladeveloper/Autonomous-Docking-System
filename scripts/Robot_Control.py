...
class RobotControl():
    ...
    def laser_ranges(self):
        time.sleep(0.1)
        self.regions = {
            'left': min(min(self.laser_msg.ranges[0:271]), 10),
            'right':  min(min(self.laser_msg.ranges[813:1083]), 10),
            'full':  min(min(self.laser_msg.ranges[0:1083]), 10),
            
        }
        print('right:',self.regions['right'])
        print('left:',self.regions['left'])
        print('full:',self.regions['full'])
    ...
    def parking_forward(self, fiducial_id):
        ...
        while self.get_laser(0) > self.parking_stop_distance:
            print(self.get_laser(0))
            trans_cam ,rotation_cam = self.get_aruco_to_robot_angle(fiducial_id)
            if abs(rotation_cam) > 0.55:
                self.cmd.angular.z = -radians(rotation_cam)
            else:
                self.cmd.angular.z = 0
            self.cmd.linear.x = 0.3
            self.publish_once_in_cmd_vel()
        #print("Robot has been parked !!!")
        self.stop_robot()
    
    def parking_check_1_point(self, fiducial_id):
        while self.get_laser(0) > self.check_1_point_stop_distance:
            trans_cam ,rotation_cam = self.get_aruco_to_robot_angle(fiducial_id)
    ...
    def pitch_correction(self, fiducial_id):
        ...
        while pitch_correction_mission == False:
            ...
            if abs(rotation_cam) > self.pitch_tolerance:
                self.cmd.angular.z = -radians(rotation_cam)
            else:
                self.cmd.angular.z = 0
                self.vel_publisher.publish(self.cmd)
