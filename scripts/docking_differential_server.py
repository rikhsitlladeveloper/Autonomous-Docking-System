...
class DockingServer:
    ...
    def __init__(self):
        self._as = actionlib.SimpleActionServer('/docking', DockingAction, execute_cb=self.on_goal,auto_start=False)
        self._as.start()
    ...
    def on_goal(self, goal):
        ...
        while self.mission == False:
            ...
            if self.mission_1 == False:
                if abs(goal_angle - self.relative_yaw_from_map_to_baselink) > 3:
                    angular = (goal_angle - self.relative_yaw_from_map_to_baselink) * -0.005
                    linear = 0
                elif abs(distance)  > 0.02:
                    angular = 0
                    linear = 0.3*abs(distance)
                else:
                    angular = 0
                    linear = 0
            ...
            elif self.mission_2 == False:
                if abs(self.relative_yaw_from_map_to_baselink  + self.initial_avg_pitch_baselink_to_aruco ) > 1:
                    angular = 0.005 * (self.relative_yaw_from_map_to_baselink  + self.initial_avg_pitch_baselink_to_aruco )
                    linear = 0
                    print(self.relative_yaw_from_map_to_baselink  + self.initial_avg_pitch_baselink_to_aruco )
                else:
                    linear = 0
                    angular = 0
                    self.mission = True
